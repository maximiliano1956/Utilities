...
int main(char argc, char *argv[]) {
	...
	if ((fp=fopen("./Steiner_snapshot.bin","rb"))!=NULL) {
		...
		if ((nbyr=fread(&nt,sizeof(unsigned int),1,fp))==0) {
			printf("Lettura dato nt errata\n");
			fclose(fp);
			exit(1);
		}
		if ((nbyr=fread(&tnb,sizeof(unsigned int),1,fp))==0) {
		...
		if ((MatDlx=allocVectChar(tnb*nt))==NULL) {
		...
		if ((nbyr=fread(MatDlx,nt*tnb,1,fp))==0) {
