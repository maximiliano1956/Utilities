...
#ifdef DEBUG		// Eventuale stampa matrice sparsa MatDlx
	printf("\n");
	printf("\n");
...
void writeSol(void) {
	...
	if ((fp=fopen(SOL_NAME,"wb+"))!=NULL && nsol>0) {
		fwrite(&b,sizeof(unsigned char),1,fp);
		fwrite(&nb,sizeof(unsigned int),1,fp);
		...
		printf("Non riesco ad creare il file delle soluzioni!\n");
}
...
void help(void) {
	printf("\n");
	printf("Optioni: -n=#numero simboli disponibili\n");
	printf("\t -b=#numero simboli in un blocco\n");
	printf("\t -t=#numero simboli nella combinazione da coprire\n");
}
