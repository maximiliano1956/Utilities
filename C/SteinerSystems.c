...
//
	...
	Scan(nb,comb_bn.numc,&comb_nbtnb,0,NULL,0);
	...
	for (int i=0;i<comb_nbtnb.numc;i++) {
		...
		for (int j=0;j<nb;j++) {
			if (verbose==1)
				printf("(");
			for (int k=0;k<b;k++) {
				soluz[nsol][j][k]=comb_bn.combnz[comb_nbtnb.combnz[i][j]-1][k];
				if (verbose==1) {
					if (k>0)
						printf(",");
					printf("%d",soluz[nsol][j][k]);
				}
			}
			cp_tnb=comb_tnb.numc;
			Scan(t,b,&comb_tnb,1,soluz[nsol][j],0);
			if (verbose==1) {
				printf(")  ");
