...
unsigned char fsol1[sizeof(int)+sizeof(int)+MAXSOL*MAXN*MAXN];	
...
void SaveSol(void) {
	...
	for (int nr=0;nr<size;nr++)
		for (int nc=0;nc<size;nc++) {
			...
			fsol1[sizeof(int)+sizeof(int)+ind]=field1[nr][nc];
			ind++;
		}
}

unsigned int FReadSol(void) {
char names[256];
FILE *fp;
int ind;
...
fread(fsol1,sizeof(int)+sizeof(int),1,fp);

size=*(unsigned int *)fsol1;
solutions=*(unsigned int *)&fsol1[sizeof(int)];

printf("Size=%d\n",size);
printf("Solutions=%d\n",solutions);

fread(&fsol1[2*sizeof(int)],solutions*size*size,1,fp);
...
for (int nr=0;nr<size;nr++)
	for (int nc=0;nc<size;nc++) {
		field1[nr][nc]=fsol1[sizeof(int)+sizeof(int)+ind];
		ind++;
	}
...
*(unsigned int *)fsol1=size;
*(unsigned int *)&fsol1[sizeof(int)]=solutions;
fwrite(fsol1,sizeof(int)+sizeof(int)+solutions*size*size,1,fp);
...
void PrintSols(void) {
	...
	for (int ns=0;ns<solutions;ns++) {
		for (int nr=0;nr<size;nr++)
			for (int nc=0;nc<size;nc++)
				field[nr][nc]=fsol1[ind++];
		show(1);
		printf("Soluzione #%d\n",ns+1);
	}
}

unsigned int WFSol(char *solname) {

	FILE *fp;

	if ((fp=fopen(solname,"w+"))==NULL) {
		printf("Errore in scrittura soluzioni\n");
		return(1);
	}
...
unsigned int WriteSol(void) {
	...
	if (esito) {
		printf("Trovata soluzione derivata\n");
		PrDer(esito);
		return(1);
	}
...
void help(void) {
	...
	printf("-pname=abcd where abcd is the puzzle name\n");
	...
	printf("-sv=abcd where abcd is the puzzle name\n");
	printf("-v verbose\n");
	printf("-h this help\n");
	printf("\n");
}
...
void PrintPar() {
	...
	for (unsigned tl=0;tl<tlist.tc;tl++) {
		sprintf(num,"%d",tlist.tiles[tl].size);
		strcat(squares,num);
		strcat(squares," ");
	}
...
unsigned int Init(int argc,char *argv[]) {
	...
	for (int ind=1;ind<argc;ind++) {
		if (!strncmp(argv[ind],OPZ1,strlen(OPZ1))) {
			...
			dck=1;
		else if (!strncmp(argv[ind],OPZ11,strlen(OPZ11))) {
			strcpy(puzzle,argv[ind]+strlen(OPZ11));
			sv=1;
		}
		else {
			help();
	...
	printf("\n");
	printf("Numero tiles=%d Area totale=%d Lato=%d\n",tlist.tc,area,size);
...
int main(int argc,char *argv[]) {
	...
	if (!result) {
		...
		if (sv) {
			if (FReadSol()) {
				printf("Errore in lettura soluzioni\n");
				return(1);
			}
