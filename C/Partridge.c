...
void AddTile(int sr,int sc,int sz) {
	...
	if (npez>maxnpez) {
		maxnpez=npez;
		printf("\nmaxnpez=%d",maxnpez);
		printf("    attempts=%d",board.attempts);
		show(1);
	}
	board.attempts++;
}
...
unsigned int ck1(unsigned int r,unsigned int c,unsigned int size,unsigned int tl) {
	if (do2 || tl!=1)
		return(1);
	...
	for (int row=sr;row<sr+board.size;row++)
		for (int col=sc;col<sc+board.size;col++) {
			for (int tl=0;tl<tlist.tc;tl++) {
				...
				if (!tlist.tiles[tl].used && (do1 || sz!=ls) && ck1(row,col,sz,tl)) {
