...
bool check_move( unsigned char* field, tiledef* tile, int d )
{
	if (mtb)
	{
		if (tile->desc->w==1 && d%2==1)
			return false;
		if (tile->desc->h==1 && d%2==0)
			return false;
	}
...
int main(int argc, char** argv)
{
	...
	for (int ind=1;ind<argc;ind++)
		{
		if (!strncmp(argv[ind],"-mtb",4))
			mtb=true;
		if (!strncmp(argv[ind],"-d",2))
			dbg=true;
		}
	...
	printf("Options: -mtb Move the block mode\n");
	printf("         -dbg Debug mode\n\n");
	printf("Move the block=%d\n",mtb);
	printf("Debug=%d\n\n",dbg);
