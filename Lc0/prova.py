...
filename = './game_000000.tar.gz'
#filename = './GameManager.tar.gz'
games_in_file = 10000
...
def main():
    with tarfile.open(filename) as f:               # descrittore del file tar
#        for idx, member in enumerate(f):            # enumerate crea una lista di coppie dove il primo indice parte
                                                    # da 0 ed e' crescente e il secondo e' l'elemento della lista f
        for member in f:                            # e' la stessa cosa
            print(member)
            if (f.extractfile(member)!=None):
                data = f.extractfile(member).read()
                bps,result = getbps_result(data)
                print(bps[0][0])
#               print(result)
    ...
    pmoves = get_training_probabilities(data, 0)
    print(pmoves)

    print(idx_to_move)
    print(len(idx_to_move))

if __name__=='__main__':

#   print(indexed_pieces)
