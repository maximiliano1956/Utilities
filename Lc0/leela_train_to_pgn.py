...
def getpgn(data, name, debugdata=None):
    # Pass an empty list for debugdata if wanted
    ...
    for moveidx in range(len(bps)-1):
        ...
        if debugdata is not None:
            move_choices = get_training_probabilities(data, moveidx)
            movenum = '{}-{}'.format(moveidx//2, moveidx%2)
            debugdata.append(dict(uci=node.move.uci(), san=node.san(), move_choices=move_choices, movenum=movenum))
    if add_final_move:
        moveidx += 1
        ...
        for idx, uci in enumerate(final_moves):
            ...
            if (board.is_insufficient_material() or
                ...
                break
            if moveidx == 449: # 450 move rule draw
                break
            board.pop()
        else:
            print()
            print("Error: Can't find final move (move index={})".format(moveidx))
            ...
            if debugdata is not None:
                movenum = '{}-{}'.format(moveidx//2, moveidx%2)
                debugdata.append(dict(uci=None, san=None, move_choices=move_choices, movenum=movenum))
            uci = None
...
def get_debug_data(tarfile_or_fileobj, name):
    if isinstance(tarfile_or_fileobj, tarfile.TarFile):
        data = tarfile_or_fileobj.extractfile(name).read()
    else:
        with tarfile.open(tarfile_or_fileobj) as f:
            data = f.extractfile(name).read()
    debugdata = []
    pgn = getpgn(data, name, debugdata)
    return {"PGN": pgn, "Moves": debugdata}
