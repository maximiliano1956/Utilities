...
#define MAXSOL 10000		// Numero massimo soluzioni memorizzate
...
QUADRATO Sol[MAXSOL];			// Lista Quadrati graeco-latini
								// trovati
...
void Search(int r, int c) {
	...
	for (nm = 0; nm < Moves.nm; nm++) {	// Ora ogni simbolo ammesso
		...
		if (r == n) {					// test se ultimo simbolo
										// depositato
			if (numsol < MAXSOL)		// In tal caso se non si sfora
				Sol[numsol] = Quadrato;	// Il quadrato viene
										// Memorizzato in una lista
...
// Controllo sul valore di n
	...
	for (ns = 0; ns < numsol; ns++) {	// Loop soluzioni trovate
		Stampa(Sol[ns]);				// Mostra il quadrato
